...
public class BeforeKeyframes {
	public static KeyFrames beforeKeyframes(AnimatedElement ae){
		...
		char last = Edge.get(edges.charAt(0)).next(rotationDirection).letterName();
		char first = Edge.get(edges.charAt(edges.length()-1)).prev(rotationDirection).letterName();
		...
		for (int i = 1; i < edges.length() - 2; i++){
			Edge e1 = Edge.get(edges.charAt(i));
			Edge e0 = Edge.get(edges.charAt(i-1));
			buildBeforeCornerKeyframe(frames, rotationDirection, e0, e1, i-1, edges.length()-2, borderWidth);
		}
	...
	public static void buildBeforeCornerKeyframe(KeyFrames frames, String rotationType, Edge e0, Edge e1, int index, int denom, int borderWidth){
		...
		double percentageTwo = buildBeforePercentage(index + 1, denom, false);
